...
subtitle: BOJ 문제 - BitMasking, BFS
...
<br>
일반적인 BFS를 진행하면 제대로 체크되지 않는다. 무한 루프 방지를 위해서 visited 체크는 필요한데, 노드만 체크할 경우 1-4-3, 1-2-3 3번 노드를 이미 방문했기 때문에 어느 한 가지는 체크가 되지 않는다. 하지만 1-4-3은 경로가 1101 이고, 1-2-3은 111로 다른 경로이다.  
<br>
...
따라서 1-4-3-2-6-2 같은 101111 이지만 **2번 노드에는 아직 이 경로가 없기 때문에 탐색 가능한 것이다.**
