---
layout: post
title: SWEA 4111 - 무선 단속 카메라
subtitle: SWEA 문제 - Disjoint-Set, Union-Find
...
Kruskal 알고리즘은 MST(Minimum Spanning Tree)를 만들기 위한 알고리즘 중 하나로이다.  
...
![최초 상태]({{ site.baseurl }}/assets/img/unionFind/union-find-1.png)

<br>
최초에는 모든 카메라에 수신기를 부여한다.
<br>

![최초 상태]({{ site.baseurl }}/assets/img/unionFind/union-find-2.png)
<br>
카메라간 거리가 가장 짧은 6, 7 카메라에 해당하는 수신기를 하나로 Union한다.

<br>

![최초 상태]({{ site.baseurl }}/assets/img/unionFind/union-find-3.png)
<br>
...
![최초 상태]({{ site.baseurl }}/assets/img/unionFind/union-find-4.png)
<br>
문제에서 주어진 수신기는 2개이기 때문에 한 번 더 수신기를 줄여 위 그림과 같은 상태를 만들어 준다.  
...
<br>
3️⃣ <span style="color:red">UnionFind를 위해서 배열을 이용하게 되면 카메라의 절대위치 값이 💥1M 이기 때문에 메모리 초과가 발생한다.</span>  
<br>
...
초기에는 parent가 자기자신은 배열이 생성된다. (0은 존재하지 않는 노드라 생각하면 된다.)
<br>
